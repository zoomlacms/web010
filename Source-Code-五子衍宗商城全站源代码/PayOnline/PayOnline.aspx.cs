                ...
                if (string.IsNullOrEmpty(PayMethod))
                {
                    ...
                    switch ((M_PayPlat.Plat)payPlat.PayClass)//现仅开通 12:支付宝即时到账和支付宝网银服务,15支付宝网银服务(上方代码中处理),银币与余额服务
                    {
                        ...
                        case M_PayPlat.Plat.WXPay:
                            #region 微信扫码支付
                            {
                                string redirect_uri = HttpUtility.UrlEncode("http://x014.2013.hx008.com/PayOnline/wxpayonline.aspx?PayNo=" + pinfo.PayNo);
                                string wxurl = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx215c3a129d4f8fcc&redirect_uri=" + redirect_uri + "&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect";
                        ...
                        case M_PayPlat.Plat.ICBC_NC:
                            #region 南昌工行
                            {
                                ...
                                //Debug End
                                //*.zoomla.cn根据需要更改如*.hx008.com
                                //string data = icbc.SpliceTranData(pinfo, payPlat.AccountID.Trim(), payPlat.SellerEmail.Trim(), "*.zoomla.cn", siteurl + "ICBCNotify.aspx");
